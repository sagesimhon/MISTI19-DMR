...
from authors_lda import LDA

class DMR(LDA):
    '''
    Topic Model with Dirichlet Multinomial Regression
    '''
    def __init__(self, T, sigma, beta, docs, vecs, V, trained=None):
        super(DMR, self).__init__(T, 0.0, beta, docs, V, trained)
        self.L = vecs.shape[1]
        self.x = vecs
        self.sigma = sigma
        self.Lambda = np.random.multivariate_normal(np.zeros(self.L),
                                                    (self.sigma ** 2) * np.identity(self.L), size=self.T)
    ...
    def get_alpha(self, Lambda=None):
        ...
        return np.exp(np.dot(self.x, Lambda.T))

    def bfgs(self):
        def ll(x):
            x = x.reshape((self.T, self.L))
            return self._ll(x)

        def dll(x):
            x = x.reshape((self.T, self.L))
            result = self._dll(x)
            result = result.reshape(self.T * self.L)
            return result

        Lambda = np.random.multivariate_normal(np.zeros(self.L),
                                               (self.sigma ** 2) * np.identity(self.L), size=self.T)
        Lambda = Lambda.reshape(self.T * self.L)

        newLambda, fmin, res = optimize.fmin_l_bfgs_b(ll, Lambda, dll)
        self.Lambda = newLambda.reshape((self.T, self.L))

    def _ll(self, x):
        result = 0.0
        # P(w|z)
        result += self.T * special.gammaln(self.beta * self.T)
        ...
        # P(Lambda)
        result += -self.T / 2.0 * np.log(2.0 * np.pi * (self.sigma ** 2))
    ...
    def _dll(self, x):
        alpha = self.get_alpha(x)
        result = np.sum(self.x[:, np.newaxis, :] * alpha[:, :, np.newaxis] \
    ...
    def params(self):
        return '''K=%d, sigma=%s, beta=%s''' % (self.T, self.sigma, self.beta)