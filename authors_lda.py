...
class LDA:
    ...
    '''
    SAMPLING_RATE = 10
    def __init__(self, T, alpha, beta, docs, V, trained=None):
        # set params
        self.T = T
        self.alpha = alpha
        self.beta = beta
    ...
    def _init_state(self):
        ...
        self.z_m_n = []
        self.n_m_z = np.zeros((len(self.docs), self.T)) + self.alpha
        self.n_z_w = np.zeros((self.T, self.V)) + self.beta
        self.n_z = np.zeros(self.T) + self.V * self.beta
    ...
    def topicdist(self):
        ...
        return self.get_alpha_n_m_z()\
            / (doclens[:, np.newaxis] + self.T * self.get_alpha())
    ...
    def params(self):
        return '''K=%d, alpha=%s, beta=%s''' % (self.T, self.alpha, self.beta)
